 ...
 */
public /*@ spec_bigint_math @*/ strictfp class FDBigInteger2 {
    ...
    private static final FDBigInteger2 POW_5_CACHE[];

    // Initialize FDBigInteger cache of powers of 5.
    static {
        POW_5_CACHE = new FDBigInteger2[MAX_FIVE_POW];
        int i = 0;
        while (i < SMALL_5_POW.length) {
            FDBigInteger2 pow5 = new FDBigInteger2(new int[]{SMALL_5_POW[i]}, 0);
            pow5.makeImmutable();
            POW_5_CACHE[i] = pow5;
            i++;
        }
        FDBigInteger2 prev = POW_5_CACHE[i - 1];
    ...
    public static final FDBigInteger2 ZERO = new FDBigInteger2(new int[0], 0);
    ...
    /*@
     ...
     @*/
    private FDBigInteger2(int[] data, int offset) {
        this.data = data;
        this.offset = offset;
    ...
    /*@
     ...
     @*/
    public FDBigInteger2(long lValue, char[] digits, int kDigits, int nDigits) {
    ...
    /*@
     ...
     @*/
    public static FDBigInteger2 valueOfPow52(int p5, int p2) {
        if (p5 != 0) {
            if (p2 == 0) {
                return big5pow(p5);
                ...
                if (bitcount == 0) {
                    return new FDBigInteger2(new int[]{pow5}, wordcount);
                } else {
                    return new FDBigInteger2(new int[]{
                            pow5 << bitcount,
                            pow5 >>> (32 - bitcount)
                    }, wordcount);
                }
    ...
    /*@
     ...
     @*/
    public static FDBigInteger2 valueOfMulPow52(long value, int p5, int p2) {
        assert p5 >= 0 : p5;
        assert p2 >= 0 : p2;
        int v0 = (int) value;
        int v1 = (int) (value >>> 32);
        ...
        if (p5 != 0) {
            if (p5 < SMALL_5_POW.length) {
                ...
                int v2 = (int) (carry >>> 32);
                if (bitcount == 0) {
                    return new FDBigInteger2(new int[]{v0, v1, v2}, wordcount);
                } else {
                    return new FDBigInteger2(new int[]{
                            v0 << bitcount,
                ...
                return (new FDBigInteger2(r, pow5.offset)).leftShift(p2);
            }
        } else if (p2 != 0) {
            if (bitcount == 0) {
                return new FDBigInteger2(new int[]{v0, v1}, wordcount);
            } else {
                return new FDBigInteger2(new int[]{
                         v0 << bitcount,
                        (v1 << bitcount) | (v0 >>> (32 - bitcount)),
        ...
        return new FDBigInteger2(new int[]{v0, v1}, 0);
    }
    ...
    /*@
     ...
     @*/
    private static FDBigInteger2 valueOfPow2(int p2) {
        int wordcount = p2 >> 5;
        int bitcount = p2 & 0x1f;
        return new FDBigInteger2(new int[]{1 << bitcount}, wordcount);
    }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() << shift);
     @*/
    public FDBigInteger2 leftShift(int shift) {
        if (shift == 0 || nWords == 0) {
            return this;
        }
        ...
        if (this.isImmutable) {
            if (bitcount == 0) {
                return new FDBigInteger2(Arrays.copyOf(data, nWords), offset + wordcount);
            } else {
                ...
                return new FDBigInteger2(result, offset + wordcount);
            }
        } else {
            if (bitcount != 0) {
                int anticount = 32 - bitcount;
    ...
    /*@
     ...
     @*/
    public int quoRemIteration(FDBigInteger2 S) throws IllegalArgumentException {
    ...
    public FDBigInteger2 multBy10() {
        ...
        if (isImmutable) {
            ...
            return new FDBigInteger2(res, offset);
        } else {
            int p = multAndCarryBy10(this.data, this.nWords, this.data);
            if (p != 0) {
                if (nWords == data.length) {
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() * pow52(p5, p2));
     @*/
    public FDBigInteger2 multByPow52(int p5, int p2) {
        if (this.nWords == 0) {
            return this;
        }
        FDBigInteger2 res = this;
        if (p5 != 0) {
            int[] r;
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() - subtrahend.value());
     @*/
    public FDBigInteger2 leftInplaceSub(FDBigInteger2 subtrahend) {
        assert this.size() >= subtrahend.size() : "result should be positive";
        FDBigInteger2 minuend;
        if (this.isImmutable) {
            minuend = new FDBigInteger2(this.data.clone(), this.offset);
        } else {
            minuend = this;
        }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() - subtrahend.value());
     @*/
    public FDBigInteger2 rightInplaceSub(FDBigInteger2 subtrahend) {
        assert this.size() >= subtrahend.size() : "result should be positive";
        FDBigInteger2 minuend = this;
        if (subtrahend.isImmutable) {
            subtrahend = new FDBigInteger2(subtrahend.data.clone(), subtrahend.offset);
        }
    ...
    /*@
     ...
     @*/
    public /*@ pure @*/ int cmp(FDBigInteger2 other) {
    ...
    /*@
     ...
     @*/
    public /*@ pure @*/ int addAndCmp(FDBigInteger2 x, FDBigInteger2 y) {
        FDBigInteger2 big;
        FDBigInteger2 small;
        int xSize = x.size();
        int ySize = y.size();
    ...
    /*@
     ...
     @*/
    private FDBigInteger2 mult(int i) {
        if (this.nWords == 0) {
            return this;
        }
        int[] r = new int[nWords + 1];
        mult(data, nWords, i, r);
        return new FDBigInteger2(r, offset);
    }
    ...
    /*@
     ...
     @*/
    private FDBigInteger2 mult(FDBigInteger2 other) {
        if (this.nWords == 0) {
            return this;
        }
        if (this.size() == 1) {
        ...
        return new FDBigInteger2(r, this.offset + other.offset);
    }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() + other.value());
     @*/
    private FDBigInteger2 add(FDBigInteger2 other) {
        FDBigInteger2 big;
		FDBigInteger2 small;
        int bigLen, smallLen;
        int tSize = this.size();
    ...
    /*@
     ...
     @ ensures \result == 0;
     @*/
    private /*@ helper @*/ long multDiffMe(long q, FDBigInteger2 S) {
        long diff = 0L;
        if (q != 0) {
     ...
     */
    private static FDBigInteger2 big5pow(int p) {
     ...
     * @return <code>5<sup>p</sup></code>.
     */
    private static FDBigInteger2 big5powRec(int p) {
        if (p < MAX_FIVE_POW) {
            return POW_5_CACHE[p];
        }
        // construct the value.
        ...
        q = p >> 1;
        r = p - q;
        FDBigInteger2 bigq = big5powRec(q);
